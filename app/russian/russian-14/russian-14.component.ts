...
export class Russian14 {
    ...
    checkInp1() {
        let check = 0;
        const arr1 = ['б', 'д', 'л', 'к', 'п', 'и', 'у', 'ё', 'а', 'о', 'я', 'е', 'м', 'ж', 'ю'];
    ...
    addEvent1() {
        ...
        imgs1.forEach(function(img) {
            img.addEventListener('click', (event) => {
                ...
                if (((index2 < 4) && (index == 0)) || ((index == 1) && (index2 > 3))) {
    ...
    addEvent8() {
        ...
        let index2: any;
        let target: HTMLElement;
        const arrDivs = ['КОМ','ЛАПА','БУЛКА','УЖ','БАБА','БЕЛКА'];
        const arrAlts = ['clod', 'pow', 'bread','snake', 'grandba', 'squirrel'];
    ...
    checkInp4() {
        let check = 0;
        const arr1 = ['булка', 'лук', 'ком', 'пол', 'мама', 'папа'];
