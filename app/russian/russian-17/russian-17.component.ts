...
export class Russian17 {
    ...
    checkInp1() {
        let check = 0;
        const arr1 = ['ю', 'д', 'л', 'п', 'м', 'я', 'у', 'ё', 'а', 'и', 'о', 'е', 'к', 'ж', 'н', 'б'];
    ...
    addEvent1() {
        ...
        const arrAlts1 = ['ns','nz'];
        const arrAlts = ['rhinoceros' ,'scissors', 'narcissus','burbot','forgetmenot','ignoramus','thread','tumbler'];
    ...
    addEvent2() {
        ...
        let index2: any;
        let target: HTMLElement;
        const arrAlts1 = ['e','yo','ls','lz'];
        const arrAlts = ['blackberry' ,'unicorn','raccoon', 'ruff','tree','hedgehog','lama','moose','onion','frog','lemon','fox'];
        ...
        imgs1.forEach(function(img) {
            img.addEventListener('click', (event) => {
                ...
                let alt = target2.getAttribute('alt');
                let index = arrAlts1.indexOf(alt!);
                if (((index == 0) && (index2 < 3)) || ((index == 1) && (index2 > 2) && (index2 < 6)) || ((index == 2) && (index2 > 5) && (index2 < 9))
                || ((index == 3) && (index2 > 8))) {
                    target2.style.border = '3px solid green';
                    setTimeout(() => target2.style.border = '1px solid black', 200)
                    target.remove();
    ...
    checkInp4() {
        let check = 0;
        const arr1 = ['нё', 'ню', 'ну', 'ни', 'на', 'но', 'не', 'ня'];
    ...
    checkInp5() {
        let check = 0;
        const arr1 = ['ёж', 'жук', 'мак', 'юла', 'банан'];
